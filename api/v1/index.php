    ...
    if (ctype_digit($_GET['count'])){
        if ($_GET['count'] <= $MAX_ALLOWED_COUNT and $_GET['count'] > 0){
            ...
            for ($x = 0; $x < $_GET["count"]; $x++) {
                ...
                array_push($image_paths,'https://api.wuffs.net/images/'.$random_number.'.jpg');
                array_push($links,'https://api.wuffs.net/?i='.$random_number);
                $valid_request = TRUE;
            } 
        } else {
            if ($_GET['count'] <= 0){
...
if ($valid_request == TRUE){
    ...
    $data = array('links' => $links, 'apiStatus' => 'non-responsive/Non-Working', 'error' => $error_msg,'status' => '405', 'msg' => 'Please use V2 of api => /api/v2');
}
